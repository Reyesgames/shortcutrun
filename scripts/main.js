    ...
    window.RuntimeInterface = class RuntimeInterface {
        ...
        async _Init(opts) {
            ...
            if (opts.runtimeBaseUrl) this._runtimeBaseUrl = opts.runtimeBaseUrl;
                ...
                this._runtimeBaseUrl = (origin ===
                    "null" ? "file:///" : origin) + location.pathname;
...
'use strict'; {
    ...
    const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
        ...
        Attach() {
            ...
            window.addEventListener("keydown", e => this._OnKeyEvent("keydown",
                e));
            ...
            if (typeof PointerEvent !== "undefined") {
                ...
                if (this._iRuntime.UsesWorker() && typeof window["onpointerrawupdate"] !== "undefined" && self === self.top) {
                    this._pointerRawUpdateRateLimiter =
                        new self.RateLimiter(() => this._DoSendPointerRawUpdate(), 5);
                ...
                window.addEventListener("touchcancel",
                    e => this._OnTouchEvent("pointercancel", e))
            }
        ...
        _OnAndroidVirtualKeyboardChange(windowHeight, vkHeight) {
            ...
            if (vkHeight > 0) {
                ...
                if (activeElement) {
                    ...
                    if (shiftY >
                        0) document.body.style.transform = `translateY(${-shiftY}px)`
                }
            }
        }
...
'use strict'; {
    ...
    function _AddShowListeners(resolver) {
        ...
        const onDismiss = event => {
            ...
            } else if (resolver) Failure(resolver,
...
'use strict'; {
    ...
    class C3Consent {
        ...
        async GetTrackingAuthorizationStatus() {
            ...
            const idfaPlugin = self["cordova"]["plugins"]["idfa"];
            const idfaInfo = await idfaPlugin["getInfo"]();
            if (!idfaInfo["trackingLimited"]) return this._trackingStatusString =
                "authorized";
        ...
        async FailureResponse(resolver,
            message) {
    ...
    async function _UpdateConsentStatus() {
        const status =
            await self["consent"]["getConsentStatus"]();
    ...
    async function _UpdateRequestLocationInEEAorUnknown() {
        ...
        if (IOS) {
            const gdprApplies = await _GetValueFromNativeStorage.call(this, "IABTCF_gdprApplies", "getInt");
    ...
    async function _RequestInfoUpdateForTesting(debug,
        debugLocation, onError) {
        let debugGeography = "";
...
'use strict'; {
    ...
    addHandler("Configure", async (id, pubID, privacyPolicy, showFree, showConsent, debug, debugLocation, showOnStartUp, fn) => {
        ...
        if (!await C3AdUtils.StartAdmobPlus(fn, debug)) return C3Consent.FailureResponse(fn, "failure to initialize");
        if (firstCall) await C3Consent.Reset();
        if (!await C3Consent.RequestInfoUpdateForTesting(debug, debugLocation, fn)) return C3Consent.FailureResponse(fn, "failure to update consent information");
        if (firstCall)
    ...
    addHandler("RequestConsent", async fn => {
        api["Configure"](C3AdUtils.ConfigurationOptions.id, C3AdUtils.ConfigurationOptions.pubID, C3AdUtils.ConfigurationOptions.privacyPolicy, C3AdUtils.ConfigurationOptions.showFree, "always", C3AdUtils.ConfigurationOptions.debug, C3AdUtils.ConfigurationOptions.debugLocation, false, fn)
    });
    ...
    addHandler("StatusUpdate", async (debug, debugLocation, fn) => {
        if (!await C3AdUtils.StartAdmobPlus(fn, debug)) return C3Consent.FailureResponse(fn, "failure to initialize");
        await C3Consent.RequestInfoUpdateForTestingSkipErrors(debug, debugLocation,
            fn);
...
'use strict'; {
    ...
    const isReady = async resolver => {
        if (!api["webAdsScriptLoaded"]) {
            C3AdUtils.Failure(resolver, "advert script not loaded, likely blocked by an ad blocker");
            return false
        }
        if (!init) {
        ...
        self["adBreak"]({
            ...
            "adBreakDone": placementInfo => {
                switch (placementInfo["breakStatus"]) {
                    ...
                    default:
                        {
                            console.table(placementInfo);C3AdUtils.Failure(fn, "interstitial ad failed to show")
                        }
                }
            }
        })
        ...
        self["adBreak"]({
            ...
            "adBreakDone": placementInfo => {
                switch (placementInfo["breakStatus"]) {
                    ...
                    default:
                        {
                            console.table(placementInfo);C3AdUtils.Failure(fn, "rewarded ad failed to show")
                        }
                }
            },
        ...
        self["adBreak"]({
            ...
            "adBreakDone": placementInfo => {
                switch (placementInfo["breakStatus"]) {
                    ...
                    default:
                        {
                            console.table(placementInfo);C3AdUtils.Failure(fn, "rewarded interstitial ad failed to show")
                        }
                }
            },
            "beforeReward": showAdFunc => {
    ...
    addHandler("Configure", async (id, pubID, privacyPolicy, showFree, showConsent, debug, debugLocation, showOnStartUp,
        fn) => {
        if (api["webAdsScriptLoaded"]) C3AdUtils.Success(fn, `UNKNOWN&&not-determined&&true`);
        else C3AdUtils.Failure(fn, "advert script not loaded, likely blocked by an ad blocker")
    });
    addHandler("RequestConsent", async fn => {
        C3AdUtils.Success(fn, `UNKNOWN&&not-determined&&true`)
    });
    addHandler("SetMaxAdContentRating", async (label, fn) => {
        C3AdUtils.Failure(fn, "setting max ad content rating not supported in web platform")
    });
    addHandler("TagForChildDirectedTreatment", async (label, fn) => {
        C3AdUtils.Failure(fn, "tagging for children not supported in web platform")
    });
    addHandler("TagForUnderAgeOfConsent", async (label, fn) => {
        C3AdUtils.Failure(fn, "tagging for under age of consent not supported in web platform")
    });
    addHandler("RequestIDFA", async fn => {
...
'use strict'; {
    ...
    const HANDLER_CLASS = class MobileAdvertHandler extends self.DOMHandler {
        constructor(iRuntime) {
            super(iRuntime, DOM_COMPONENT_ID);
            const actionHandler = name => [name, data => this._CallMethod(name, data)];
            const addScriptHandler = name => [name, data => this._AddScript(data[0], data[1])];
            this.AddRuntimeMessageHandlers([actionHandler("CreateBannerAdvert"), actionHandler("ShowBannerAdvert"), actionHandler("HideBannerAdvert"), actionHandler("CreateInterstitialAdvert"),
                actionHandler("ShowInterstitialAdvert"), actionHandler("CreateVideoAdvert"), actionHandler("ShowVideoAdvert"), actionHandler("CreateRewardedAdvert"), actionHandler("ShowRewardedAdvert"), actionHandler("CreateRewardedInterstitialAdvert"), actionHandler("ShowRewardedInterstitialAdvert"), actionHandler("Configure"), actionHandler("RequestConsent"), actionHandler("SetUserPersonalisation"), actionHandler("SetMaxAdContentRating"), actionHandler("TagForChildDirectedTreatment"), actionHandler("TagForUnderAgeOfConsent"),
                actionHandler("RequestIDFA"), actionHandler("StatusUpdate"), addScriptHandler("AddScript")
            ])
        }
        _GetApi() {
            if (self["cordova"]) return self.C3MobileAdvertsAPI.real;
            else if (!self["cordova"] && !USE_EMULATOR) return self.C3MobileAdvertsAPI.web;
        ...
        async _CallMethod(name, data) {
            ...
            if (!api["suspendRuntime"]) api["suspendRuntime"] = () => this._SuspendRuntime();
            if (!api["resumeRuntime"]) api["resumeRuntime"] = () => this._ResumeRuntime();
            if (!api) {
                if (!hasShownWarning) {
        ...
        async _AddScript(url, attributes = null) {
            ...
            return new Promise((resolve, reject) => {
                ...
                if (attributes)
                    for (let attribute in attributes) scriptElem.setAttribute(attribute, attributes[attribute]);
